...
crs.sum1<-read.csv("input-dataset/Class-Fake.csv", stringsAsFactors = F)
crs.sum2<-read.csv("input-dataset/Class2-Fake.csv", stringsAsFactors = F)
crs.df2<-crs.sum2
crs.df1<-crs.sum1

outfilename<-"Fake-Two-Class"
...
#restrict<-data.demo$FirstMath==firstmath

#crs.df<-crs.df[restrict,]
#enroll.df<-enroll.df[restrict,]
#data.demo<-data.demo[restrict,]

#cat("Restricted Dataset", nrow(enroll.df), "\n")
...
# Process Course 1
for(i in 1:nrow(crs.df1)){
  state<-1
  for(j in 1:ncol(crs.df1)){
    if(j==1){
      if(crs.sum1[i,j]>=passing.grade){
        crs.df1[i,j]<-3
      }
      if(crs.sum1[i,j]<passing.grade && crs.sum1[i,j]>=0 ){
        crs.df1[i,j]<-2
      }
      if(crs.sum1[i,j]<0){
        crs.df1[i,j]<-1
...
# Process Course 2
for(i in 1:nrow(crs.df2)){
  state<-1
  for(j in 1:ncol(crs.df2)){
    if(j==1){
      if(crs.sum2[i,j]>=passing.grade){
        crs.df2[i,j]<-3
      }
      if(crs.sum2[i,j]<passing.grade && crs.sum2[i,j]>=0 ){
        crs.df2[i,j]<-2
      }
      if(crs.sum2[i,j]<0){
        crs.df2[i,j]<-1
...
crs.states<-c("NT", "F", "P")
crs.states.num<-1:3
crs.states.prod<-c("NT1NT2", "NT1F2", "NT1P2", "F1NT2", "F1F2", "F1P2", "P1NT1", "P1F2", "P1P2")
crs.states.num.prod<-1:9

#
# Form the product matrix
#

crs.df<-crs.df1 # to get matrix size right

for(i in 1:nrow(crs.df)){
  for(j in 1:ncol(crs.df)){
    crs.df[i,j]<-3*(crs.df1[i,j]-1)+crs.df2[i,j]
  }
}
...
for(j in 2:last.sem){
  for(k in 1:length(crs.states.prod)){
    ...
    state.i<-paste(enroll.state[1], crs.states.prod[k], j, sep="")
    node.count<-node.count+1
    nodes[node.count,]<-NA
    nodes$state[node.count]<-state.i
...
node.count<-node.count+1
nodes[node.count,]<-NA
nodes$state[node.count]<-paste(enroll.state[1], crs.states.prod[1], 1, sep="")
...
for(k in 1:length(crs.states.prod)){
  ...
  edges[edge.count,]<-NA
  edges$state.i[edge.count]<-"EMNT1NT21"
  edges$state.f[edge.count]<-paste(crs.states.prod[k], "1", sep="")
  cnt<-nrow(crs.df[crs.df[,1]==k,])
...
for(j in 1:last.sem){
  for(k in 1:length(crs.states.num.prod)){
    cat("j", j, "k", k, "\n")
    ...
    state.i<-paste(crs.states.prod[k], j, sep="")
    node.count<-node.count+1
    nodes[node.count,]<-NA
    nodes$state[node.count]<-state.i
    nodes$count[node.count]<-nrow(reduced.crs)
    ...
    if(j!=last.sem){
      ...
      for(m in 1:length(enroll.state)){
        ...
        if(m>1){
          ...
          if(nrow(reduced.en.crs2)==0) next
          #cat("Reduced2", m, nrow(reduced2), "\n")
          state.f<-paste(enroll.state[m], crs.states.prod[k],  next.sem, sep="")
          edge.count<-edge.count+1
          edges[edge.count,]<-NA
          edges$state.i[edge.count]<-state.i
          edges$state.f[edge.count]<-state.f
...
for(j in 2:last.sem){
  ...
  reduced<-crs.df[enroll.df[,j]==1,]
  cat("student enrolled", k, nrow(reduced), nrow(reduced.en), "\n")
  for(m in 1:length(crs.states.prod)){
    prev.sem<-j-1
    state.i<-paste(enroll.state[1], crs.states.prod[m], j, sep="")
    ...
    for(k in 1:length(crs.states.num.prod)){
      ...
      if(nrow(reduced.crs)==0) next
      #cat("Reduced2", m, nrow(reduced2), "\n")
      state.f<-paste(crs.states.prod[k], j, sep="")
      edge.count<-edge.count+1
      edges[edge.count,]<-NA
      edges$state.i[edge.count]<-state.i
      edges$state.f[edge.count]<-state.f
